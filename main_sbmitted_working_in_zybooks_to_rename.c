...
void if_new_line_remove_last_char(char *s);
...
//Prompt user for title for data and output the title
   ...
   dataTitle[strlen(dataTitle)-1]='\0';
...
data_node * build_data_list(void) {
   ...
   char buf[MAX_LEN]="";
   int i_l, i_c, list_size=0, num_entries =0;
   ...
   while(strcmp(buf, "-1")!=0){
      ...
      last[0]='\0';
      buf[0]='\0';
      if(num_entries == 0){fgets(buf, MAX_LEN, stdin);
        if(strcmp(buf, "-1")==0){break;}}
      printf("Enter a data point (-1 to stop input):\n");
      fflush(stdin);
      if(num_entries!=0){fgets(buf, MAX_LEN, stdin);}
      if(strcmp(buf, "-1")==0){break;}
      if(no_commas(buf)==1){parse(buf, &i_l, &i_c, first, last);}
      num_entries++;
      while(no_commas(buf)!=1 || isnum(first)!=1){//strchr(buf, ',')==NULL
         ...
         printf("Enter a data point (-1 to stop input):");
         fflush(stdin);
         if(num_entries!=0){fgets(buf, 100, stdin);}
         if(strcmp(buf,"-1")==0){break;}
         printf("\n");
         if(no_commas(buf)==1){parse(buf, &i_l, &i_c, first, last);}
         num_entries++;
   ...
   if(num_entries ==0 || list_size==0){return NULL;}
   else{ return headObj; }
}

void PrintTableHeader(char *title, char *col1, char *col2){
   int i, middex= (strlen(title)/2+1), number_spaces_preceding_title;
